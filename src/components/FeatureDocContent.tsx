...
/**
 * Client component that renders feature doc HTML with optional inline editing.
 * The editing feature gracefully degrades when the Go API is unavailable.
 */
export default function FeatureDocContent({
  product,
  version,
  locale,
  slug,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSaveResult({
          success: false,
          message: result.message || "Failed to save. The editing API is not available yet.",
        });
      }
    } catch {
      setSaveResult({
        success: false,
        message: "Editing is not available yet. The API will be deployed soon.",
      });
    } finally {
      setIsSaving(false);
    }
